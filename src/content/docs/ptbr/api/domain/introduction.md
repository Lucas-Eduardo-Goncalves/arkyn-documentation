...
- [**TrafficSource**](/ptbr/api/trafficsource/introduction): Cada `Domain` pertence a uma única `TrafficSource`.
- [**Pathname**](/ptbr/api/pathname/introduction): Um `Domain` pode ter vários `Pathname` (caminhos de URL) associados a ele.
- [**HttpTraffic**](/ptbr/api/httptraffic/introduction): Armazena os registros de tráfego HTTP (`HttpTraffic`) que ocorreram nesse domínio específico.
