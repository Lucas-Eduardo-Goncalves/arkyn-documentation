...
- [**TrafficSource**](/ptbr/api/trafficsource/introduction): Cada `Pathname` está associado a uma `TrafficSource` para facilitar a agregação de dados no nível da fonte de tráfego.
- [**Domain**](/ptbr/api/domain/introduction): Cada `Pathname` pertence a um `Domain` específico.
- [**HttpTraffic**](/ptbr/api/httptraffic/introduction): Um `Pathname` pode ter múltiplos registros de `HttpTraffic` associados, representando todas as requisições feitas para aquele caminho específico.
