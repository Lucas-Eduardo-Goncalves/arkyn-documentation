...
- [**User**](/ptbr/api/user/introduction): Cada fonte de tráfego pertence a um único usuário.
- [**Domain**](/ptbr/api/domain/introduction): Uma `TrafficSource` pode ter vários `Domain` (domínios) associados.
- [**Pathname**](/ptbr/api/pathname/introduction): Agrupa todos os `Pathname` (caminhos de URL) registrados sob esta fonte.
- [**HttpTraffic**](/ptbr/api/httptraffic/introduction): Armazena os registros de tráfego HTTP (`HttpTraffic`) gerados.
- [**CorePathname**](/ptbr/api/corepathname/introduction): Mantém os caminhos de URL principais (`CorePathname`) para agregação de logs.
- [**CoreLog**](/ptbr/api/corelog/introduction): Consolida os logs principais (`CoreLog`).
