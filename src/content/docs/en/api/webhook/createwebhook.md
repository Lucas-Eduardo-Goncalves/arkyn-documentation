...
This route requires authentication. You must include a valid Bearer token in the request's `Authorization` header. The token must be obtained through the [user authentication route](/en/api/user/authuser/).
