...
- [**TrafficSource**](/en/api/trafficsource/introduction): Each `Pathname` is associated with a `TrafficSource` to facilitate data aggregation at the traffic source level.
- [**Domain**](/en/api/domain/introduction): Each `Pathname` belongs to a specific `Domain`.
- [**HttpTraffic**](/en/api/httptraffic/introduction): A `Pathname` can have multiple `HttpTraffic` records associated with it, representing all requests made for that specific path.
