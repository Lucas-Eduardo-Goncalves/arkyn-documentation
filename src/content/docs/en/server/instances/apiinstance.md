---
title: ApiInstance
description: A class that represents an API instance to handle HTTP requests with base configurations.
---

The `ApiInstance` class is used to create an instance that manages HTTP requests, allowing the configuration of a base URL, headers, and authorization token to be used in all calls.
...
- **props** (`ApiInstanceContructorProps`): The configuration properties for the API instance.
...
The `ApiInstance` provides methods for the most common HTTP verbs. All methods return a `Promise` that resolves to an `ApiResponseDTO` object.
...
Each request method (`GET`, `POST`, etc.) of the `ApiInstance` uses a corresponding helper function (`getRequest`, `postRequest`, etc.). These functions, in turn, are wrappers for the central `makeRequest` function, which is responsible for executing the `fetch` call and standardizing the response.
...
1.  **Gets Configuration**: Retrieves the log API URL and user token from `ArkynLogInstance`. If the configuration is not available, the function does nothing.
...
2.  The `PATCH` method of `ApiInstance` calls `patchRequest`.
